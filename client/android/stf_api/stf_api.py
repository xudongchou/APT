...
import json
import time
from Conf.stf_conf import user_device_url,token,device_url
...
class StfDevice:
    def __init__(self):
        ...
        self.user_device_url = user_device_url
        self.token = token

    def __get_all_devices_info(self):
        headers = {"Authorization": "Bearer" + self.token}
        result = requests.get(url=self.device_url, headers=headers)
        return result

    def __get_user_device_info(self):
        headers = {"Authorization": "Bearer" + self.token}
        result = requests.get(url=self.user_device_url, headers=headers)
        return result
